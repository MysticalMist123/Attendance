...
function update(sub_id,date,new_value,callback){
    fs.readFile(path,(err,DATA)=>{
        ...
        let old_value = data[sub_id]['dates'][date]
        data[sub_id]['dates'][date] = new_value

        console.log(data[sub_id]['present'],new_value,old_value)
        data[sub_id]['present'] += new_value - old_value
...
// NOTE: In update, the total number of classes on that daywould remain the same as it was before the updation.
//       the total will only change if the new value is more than total, only then total will be changed.
//       Which means, if you have set the total number of classes initially (before updation) to a unintended value, then it can't be changed to a lower value.
//       This is one disadvantage (and due to poor planning)